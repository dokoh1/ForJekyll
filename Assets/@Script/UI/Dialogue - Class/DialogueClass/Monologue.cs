...
[Serializable]
    public struct Monologue : IDialogue, ILog
    {
        ...
        public Monologue(int next, int background)
        {
            _next = next;
        ...
        {
            var mono = this;
            commands.Enqueue(SoundManager.Instance.PlayDialogueSE);
            commands.Enqueue(() => view.ChangeDialogueType(EDialogueType.Monologue));
            commands.Enqueue(() => view.SetScript(mono._context[DataManager.Instance.LocaleSetting]));
            commands.Enqueue(() => view.SetImage("", null));
            commands.Enqueue(() => view.RequestJump(mono._next));
            commands.Enqueue(() => mono.CreateLog(view));
